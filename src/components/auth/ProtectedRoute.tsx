        ...
        return (
            <div className="min-h-screen bg-cream flex flex-col items-center justify-center p-6 text-center">
                ...
                <button
                    onClick={async () => {
                        try {
                            console.log('Solicitando cierre de sesión forzado...');
                            await signOut();
                        } catch (err) {
                            console.error('Error al intentar salir:', err);
                            // Fallback extremo
                            window.location.replace('/admin/login');
                        }
                    }}
                    className="bg-chocolate text-cream px-10 py-4 text-[10px] uppercase tracking-widest font-bold hover:bg-gold transition-all shadow-xl active:scale-95 flex items-center gap-2"
                >
